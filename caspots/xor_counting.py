...
class Propagator:
    ...
    def init(self, init):
        ...
        dnf = [init.solver_literal(atom.literal) for atom in init.symbolic_atoms.by_signature(
            "dnf", 2)]  # if abs(init.solver_literal(atom.literal)) != 1]
        clause = [init.solver_literal(atom.literal) for atom in init.symbolic_atoms.by_signature(
            "clause", 3)]  # if abs(init.solver_literal(atom.literal)) != 1] #
        ...
        if len(literals) > 0:
            ...
            for i in range(estimated_s):
                ...
                for thread_id in range(init.number_of_threads):
                    self.__states[thread_id].setdefault(
                        index, []).append((lits, parity))
                index += 1
            # print self.__states
