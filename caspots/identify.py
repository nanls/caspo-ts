...
from caspots import asputils, xor_counting
...
# from crossvar import globalvariables # not used...
...
class ASPSolver:
    ...
    def solutions(self, on_model, on_model_weight=None, limit=0,
        ...
        control.add("minWeight", [], ":- not " + str(weight) + " #sum {Erg,E,T,S : measured(E,T,S,V), not guessed(E,T,S,V), toGuess(E,T,S), obs(E,T,S,M), Erg=50-M, M < 50;" +
                    " Erg,E,T,S : measured(E,T,S,V), not guessed(E,T,S,V), toGuess(E,T,S), obs(E,T,S,M), Erg=M-49, M >= 50} " + str(max_weight) + " .")
        ...
        if do_mincard:
            ...
            control.add("minSize", [], ":- not " + str(minsize) +
                ...
                for atom in atoms:
                    ...
                    if n == "dnf" and len(a) == 2:
                        ...
                        control.assign_external(
                            clingo.Function("before", a), True)
                        ctx.dnf_args.append(tuple(a))
                for atom in natoms:
                    n, a = atom.name, atom.arguments
                    if n == "dnf" and len(a) == 2:
                        control.assign_external(
                            clingo.Function("before", a), False)
        ...
        if self.opts.xor is not None:
            control.register_propagator(xor_counting.Propagator(self.opts.xor))
