...
def do_identify(args):
    ...
    def show_stats(output=sys.stderr):
        if args.true_positives:
            output.write("%d solution(s) / %d true positives\r" %
                         (c["found"], c["tp"]))
        else:
    ...
    def on_model(model):
        ...
        tuples = ([x.number for x in f.arguments]
                  for f in model.symbols(shown=True) if f.name == "dnf")
     ...
     #Misbah'''
    try:
        identifier.solutions(on_model, limit=args.limit,
...
def do_validate(args):
    ...
    try:
        ...
        res = "%d/%d true positives [rate: %0.2f%%]" % (
            tp, nb, (100. * tp) / nb)
...
def run():
    ...
    domain_parser.add_argument(
        "--networks", help="Networks to as domain (.csv)")
    ...
    parser_results2lp.add_argument(
        "networks", help="Networks file (.csv format)")
